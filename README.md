...
The account whose verified email matches `APP_OWNER_EMAIL` gets an **Administration** tab in Settings, gated server-side by `requireOwner()`. Administrators retain the user list, account details (including last sign-in), password-reset emails, session revocation, disable/re-enable and deletion controls, AI access and budget management, usage totals, API costs, and administrative audit logs.
