...
The account whose verified email matches `APP_OWNER_EMAIL` gets an **Administration** tab in Settings (never a separate `/admin` route or login) — gated server-side by `requireOwner()` (`src/lib/admin/roles.ts`), never by a client-computed email check. From there the owner can, per user: grant/revoke/suspend/reactivate AI parser access, set daily-request and monthly-budget limits, view usage totals and recent parser activity metadata, send a Supabase password-reset email, rotate their password to terminate refresh sessions, and disable/re-enable the account. Already-issued access JWTs remain usable until their short expiry, so configure an appropriately short Supabase JWT lifetime. Open **Users & AI Access → a user → Applications & parsing** to inspect that user’s saved/submitted applications (including archived records) and accepted AI parsing attempts. Applications include company, role, location, status, dates, compensation, contacts, notes, and posting text. Parsing history includes outcomes, errors, model/cost/token data, submitted posting text, and the extracted result when retained. Parsing a posting is not evidence that an application was submitted. The view is read-only, paginated, and owner-only. Application details show the currently linked resume and an **Open resume in new tab** link. The owner-only application resume route verifies both application ownership and resume ownership, checks upload status and the storage key, rate-limits access, audits each open, and redirects to a short-lived inline B2 URL. PDFs open in browsers that support PDF viewing; Word files may download depending on browser support. No resume snapshot is retained if the user changes or deletes the linked resume. No impersonation is provided. Each activity page read and successful administrative mutation is recorded in `admin_audit_log`; activity responses use `Cache-Control: private, no-store`.

Apply `supabase/migrations/20260911120000_admin_user_activity.sql` before deploying this feature (new installations can use the canonical schema). It adds service-only `ai_parser_attempt_details`, linked to usage rows with cascading deletion, to retain the submitted job description/URL and extracted result for new accepted requests. Older requests may only have usage metadata; rejected requests before slot acquisition (invalid input, access denial, or rate/budget limits) are not parsing attempts. Context-write failures are logged without posting content and do not interrupt parsing or usage finalization. Ordinary users cannot access the admin view or other users’ records; existing application RLS policies remain in force.
